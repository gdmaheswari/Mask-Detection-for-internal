...
PATH_TO_CKPT_person = TRAINED_MODEL_DIR + '/model_person/frozen_inference_graph.pb'
...
def load_inference_graph():
    ...
    return detection_graph1,detection_graph2,sess1,sess2


def draw_box_on_image(num_person_detect, score_thresh, scores, boxes, classes, im_width, im_height, image_np,Line_Position2,Orientation):
    ...
    color0 = (255,0,0)
    color1 = (0,50,255)
    for i in range(num_person_detect):
        ...
        if (scores[i] > score_thresh):
            ...
            cv2.putText(image_np, 'Person '+str(i)+': '+id, (int(left), int(top)-5),
...
def detect_objects(image_np, detection_graph1, sess1):
    # Definite input and output Tensors for detection_graph
    image_tensor = detection_graph1.get_tensor_by_name('image_tensor:0')
    # Each box represents a part of the image where a particular object was detected.
    detection_boxes = detection_graph1.get_tensor_by_name(
        'detection_boxes:0')
    # Each score represent how level of confidence for each of the objects.
    # Score is shown on the result image, together with the class label.
    detection_scores = detection_graph1.get_tensor_by_name(
        'detection_scores:0')
    detection_classes = detection_graph1.get_tensor_by_name(
        'detection_classes:0')
    num_detections = detection_graph1.get_tensor_by_name(
        'num_detections:0')

    image_np_expanded = np.expand_dims(image_np, axis=0)

    (boxes1, scores1, classes1, num) = sess1.run(
        [detection_boxes, detection_scores,
            detection_classes, num_detections],
        feed_dict={image_tensor: image_np_expanded})
    return np.squeeze(boxes1), np.squeeze(scores1), np.squeeze(classes1)


def detect_objects1(image_np, detection_graph2, sess2):
    # Definite input and output Tensors for detection_graph
    image_tensor = detection_graph2.get_tensor_by_name('image_tensor:0')
    # Each box represents a part of the image where a particular object was detected.
    detection_boxes = detection_graph2.get_tensor_by_name(
        'detection_boxes:0')
    # Each score represent how level of confidence for each of the objects.
    # Score is shown on the result image, together with the class label.
    detection_scores = detection_graph2.get_tensor_by_name(
        'detection_scores:0')
    detection_classes = detection_graph2.get_tensor_by_name(
        'detection_classes:0')
    num_detections = detection_graph2.get_tensor_by_name(
        'num_detections:0')
